...
## 16. Deferred behavior-changing items from the dedupe/cleanup refactor

`documents/RefactoringPlan.md` (2026-07-05, branch `feature/refactor-dedupe`) scoped its refactor to behavior-unchanged
...
`index.html` still has `lang="en"` and `title="temp-scaffold"`. The app is Japanese-first and no longer a scaffold;
these should be updated (e.g. `lang="ja"` and a real title) in a small, standalone UI/content commit.

### Static JSON generation is GitHub Actions-only

News and Traffic JSON (`public/data/news.json`, `public/data/traffic.json`) are currently generated by GitHub Actions
cron as part of `deploy-pages.yml` (see section 7, "Artifact-only refresh"). A local-server migration needs an
