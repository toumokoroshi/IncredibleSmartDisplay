...
function hasNumberArrays(record: Record<string, unknown>, keys: string[], minLength: number) {
  return keys.every((key) => isNumberArray(record[key]) && record[key].length >= minLength);
}

function hasStringArrays(record: Record<string, unknown>, keys: string[], minLength: number) {
  return keys.every((key) => isStringArray(record[key]) && record[key].length >= minLength);
}

function isOpenMeteoResponse(value: unknown): value is ValidOpenMeteoResponse {
  if (!isRecord(value) || !isRecord(value.current) || !isRecord(value.daily) || !isRecord(value.hourly)) {
    return false;
  }
  ...
  if (requiredCurrentKeys.some((key) => typeof current[key] !== "number")) {
    return false;
  }

  const daily = value.daily;
  const hourly = value.hourly;
  const dailyLength = isStringArray(daily.time) ? daily.time.length : 0;
  const hourlyLength = isStringArray(hourly.time) ? hourly.time.length : 0;

  return (
    dailyLength >= 2 &&
    hourlyLength > 0 &&
    hasStringArrays(daily, ["time", "sunrise", "sunset"], dailyLength) &&
    hasNumberArrays(
      daily,
      [
        "weather_code",
    ...
    hasStringArrays(hourly, ["time"], hourlyLength) &&
    hasNumberArrays(
      hourly,
      [
        "temperature_2m",
...
async function readOpenMeteoPayload(response: Response) {
  let payload: unknown;
  try {
    payload = await response.json();
  } catch {
    throw createServiceError("DATA_INVALID", "DATA_INVALID", false);
  }
...
function mapOpenMeteoWeather(payload: ValidOpenMeteoResponse, settings: WeatherSettings): WeatherData {
  const conditionCode = payload.current.weather_code;
...
function mapDailyForecast(payload: ValidOpenMeteoResponse, currentHumidity?: number, currentWindDirection?: number): WeatherDailySummary[] {
...
function mapHourlyForecast(payload: ValidOpenMeteoResponse) {
