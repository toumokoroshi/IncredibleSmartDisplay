...
export function HeaderBar({ status, title }: { status: HeaderStatus; title: string }) {
  return (
    <header className="col-span-2 rounded-lg border border-white/10 bg-slate-950/50 px-6 py-3">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap justify-end gap-2 text-right text-sm text-slate-300">
...
function StatusPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-28 rounded-lg border border-white/10 bg-white/5 px-3 py-2">
